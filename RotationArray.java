...
Q1 is solved by the function rotation_recursive() and/or rotation_iterative(). Both can solve the problem correctly; 
the difference is that one is implemented with recursion while the other one uses loop. 
...
Q2 is solved by the function rotation_and_find_recursive() and/or rotation_and_find_iterative(). 
Again, both can solve the problem correctly; 
the difference is that one is implemented with recursion while the other one uses loop. 
Our implementation applies to the cases when:
    1) the array has duplicates (i.e., non-strictly monototically ascending with rotation);
